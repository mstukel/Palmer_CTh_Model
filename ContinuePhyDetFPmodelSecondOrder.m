...
load('Phy-Det-FP model Second Order Outputs.mat')

% k_sorp_phy = 0.013;                 %m3 / mmol C / d, Stukel et al. (2019)
% k_sorp_det = 0.013;                 %m3 / mmol C / d, Stukel et al. (2019)
% k_desorp = 2/365;                 %Lerner
% Eg_Th = 0.5;
starttimes = [9.5,50];
endtimes = [40,137];
% jmp_k_sorp_phy = k_sorp_phy/30;
...
% [norm_misfit0] = PhyDetFPmodelSecondOrder(k_sorp_phy,k_sorp_det,k_desorp,Eg_Th,starttimes,endtimes,0);
% 
...
for i=1:numiter
    ...
    [proposal_norm_misfit] = PhyDetFPmodelSecondOrder(proposal_k_sorp_phy,proposal_k_sorp_det,proposal_k_desorp,proposal_Eg_Th,starttimes,endtimes,0);
    ...
    if rand<prob
        ...
        accepted = accepted+1;
        k_sorp_phy = proposal_k_sorp_phy;
    ...
    k_sorp_phy_track(end+1) = k_sorp_phy;
    ...
    if mod(i,1000)==0
        origin = 'RunPhyDetFPmodelSecondOrder.m'
        save('Phy-Det-FP model Second Order Outputs.mat','origin','k_sorp_phy_track','k_sorp_det_track','k_desorp_track','Eg_Th_track','loglike_track','prior_track',...
            'accepted','rejected','k_sorp_phy','k_sorp_det','k_desorp','Eg_Th','loglike0','norm_misfit0','prior0',...
            'jmp_k_sorp_phy','jmp_k_sorp_det','jmp_k_desorp','jmp_Eg_Th','mu_k_sorp_phy','mu_k_sorp_det','mu_k_desorp','sig_k_sorp_phy','sig_k_sorp_det','sig_k_desorp','alpha_Eg_Th','beta_Eg_Th')
    end
    time1=toc
end
...
subplot(2,3,2)
plot(k_sorp_phy_track)
title('k_s_o_r_p_,_p_h_y')
