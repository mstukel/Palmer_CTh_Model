function [norm_misfit] = POCFPmodelFirstOrder(k_sorp,k_desorp,Eg_Th,starttimes,endtimes,toplot)
...
for i=find(time==starttimes(1)):find(time==endtimes(1))
    ...
    %Sorption
    PTh = PTh + k_sorp.*dTh*dt;
    dTh = dTh - k_sorp.*dTh*dt;
...
if toplot==1
    ...
    subplot(3,1,3)
    pcolor(time,z,POC_track-POC_hold)
    shading flat
    colorbar
    axis ij
    caxis([-0.01 0.01])
    ...
    colorbar
    axis ij
    title('Total Thorium (dpm L^-^1) - POC-FP-Model 1^s^t Order')
    ...
    colorbar
    axis ij
    title('Fraction Particulate Thorium - POC-FP-Model 1^s^t Order')
    ...
    colorbar
    axis ij
    title('Carbon:Thorium (umol C dpm^-^1) - POC-FP-Model 1^s^t Order')
    hold on
    ...
    ylabel('C:Th Ratio (umol dpm^-^1) - POC-FP-Model 1^s^t Order')
